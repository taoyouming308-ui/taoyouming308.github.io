...
export function renderOrderFlow(container,flow,locked,onAction){
 ...
 if(flow){
  const hint=doc.createElement('p');hint.textContent=locked?'有未保存修改或版本冲突，请先处理；不能推进状态。':flow.state==='awaiting_payment'?'当前待收银；本页尚无收款入口，未发生会员扣款。':'只推进整单状态，不自动完成项目明细，也不产生收款或出库。';fragment.append(hint);
