...
|---|---|---|
| G01 | 原 13 模块仍分别读写 localStorage；本机工作台已连通建档/开单、带版本的新建/恢复/列表重选草稿多项目编辑、本店状态筛选分页、整单开单/服务/待收银、四类同标签页核对及内部编号原单只读查询；Auth 仍为合成替身 | 确认专用测试项目及登录方式、接入正式 SDK 并逐模块迁移；补订单日期/搜索、改价优惠员工分配、项目服务完成、收银页面、其他操作及跨设备恢复、完整页面端到端测试 |
