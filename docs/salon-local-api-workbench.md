...
退款审批已接入：本店申请分页 → 读取原单/原支付分配 → 双人批准/拒绝 → 回读确认；快照变化拒绝旧审批，未知结果同键/只读恢复。只处理已有申请，默认角色无审批权限，专项测试使用独立合成双人夹具。批准不等于已退款；无退款申请或执行按钮。见 `salon-refund-review.md`。

现金全额收银已接入：载入待收银单 → 预览 → 人工确认 → 版本化提交 → 按原请求核对具体支付记录 → 读取订单现状。支持丢包同键重试/刷新只读恢复；默认夹具未放宽权限或库存，成功提交由专项测试独立设置合成夹具。详见 `salon-cash-checkout.md`。不接会员/混合/电子支付。
